...
class QRScannerRectView extends Component {
  ...
  getRect = () => {
    return new Promise((resolve, reject) => {
      const handle = findNodeHandle(this._scanView);
...
export default class QRScannerView extends Component {
  ...
  getCameraRect = () => {
    return new Promise((resolve, reject) => {
      const handle = findNodeHandle(this._camera);
      UIManager.measureInWindow(handle, (x, y, w, h) => {
        resolve({ x, y, w, h });
      });
    });
  ...
  tranRect = async data => {
    const cameraRect = await this.getCameraRect();
    const cameraModalRect = await this._cameraModal.getRect();
    const x = (cameraModalRect.x / cameraRect.w) * data.width;
    const y = (cameraModalRect.y / cameraRect.h) * data.height;
    const w = (cameraModalRect.w / cameraRect.w) * data.width;
    const h = (cameraModalRect.h / cameraRect.h) * data.height;
    return { offset: { x, y }, size: { width: w, height: h } };
  };
  ...
  takePictureAsync = option => {
    ...
    return new Promise(async (resolve, reject) => {
      const cameraModalRect = await this._cameraModal.getRect();
      const data = await this._camera.takePictureAsync(option);
      await this._camera.pausePreview();
      Image.getSize(data.uri, async (w, h) => {
        cropData = await this.tranRect({ width: w, height: h });
        cropData.displaySize = {
          width: cameraModalRect.w,
          height: cameraModalRect.h
        };
        // cropData.resizeMode = "cover";
        ImageEditor.cropImage(
          data.uri,
          cropData,
          uri => {
            RNFS.readFile(uri, "base64").then(e => {
              this._camera.resumePreview();
              resolve({ base64: e, uri });
            });
          },
          data => {
            console.log("object");
      ...
      <View style={{ flex: 1 }}>
        <Camera
          onBarCodeRead={r =>
            ...
            this.props.onScanResultReceived(r)
          }
          style={{ flex: 1 }}
          ref={r => (this._camera = r)}
        >
          {/*绘制顶部标题栏组件*/}
